...
export function PaginaSenhas() {
    const { obterItem, removerItem, copiarItem } = Armazenamento();
    ...
    async function copiarToken(item) {
        const tokens  = await copiarItem("@token", item)
        defListaTokens(tokens)
            ...
            <View style={ESTILO.content}>
                <FlatList
                    ...
                    renderItem={({ item }) => <CaixaToken
                        ...
                        transferirToken={() => copiarToken(item)}
                    />}
                />
            </View>
        </SafeAreaView>
